package com.qart.ploticalparties.testutils;
...
public class TestUtils {
    ...
    public static int failed;
		...
		public static void myAssert(String testName, Object result, File file) throws IOException {
			total++;
			String[] r = testName.split("(?=\\p{Upper})");
			System.out.print("\n"  + BLUE_BOLD_BRIGHT + "TEST CASE => ");
			
			System.out.print(GREEN_BOLD_BRIGHT + "Test For " + " ");

			if(file.getName().contains("output_revised")){
                System.out.print(GREEN_BOLD_BRIGHT + "[FUNCTIONALITY]" + " ");
            }
            if(file.getName().contains("exception")){
                System.out.print(GREEN_BOLD_BRIGHT + "[EXCEPTION]" + " ");
            }
            if(file.getName().contains("boundary")){
                System.out.print(GREEN_BOLD_BRIGHT + "[BOUNDARY]" + " ");
            }
			
			for(int i=1;i<r.length;i++) {
				System.out.print(GREEN_BOLD_BRIGHT + r[i] + " ");
			}
			System.out.print(" : ");
